...
<body class="font-sans antialiased bg-gradient-to-br from-gray-900 via-black to-gray-800">
    <div class="min-h-screen flex flex-col">
        <!-- Navigation -->
        @auth
        <nav class="bg-black/50 backdrop-blur-md border-b border-gray-800 shadow-2xl">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center">
                        <h1 class="text-xl font-bold text-white">
                            <a href="{{ route('dashboard') }}" class="hover:text-gray-300 transition-colors">Expense Tracker</a>
                        </h1>
                    </div>
                    <div class="flex items-center space-x-4">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="inline" id="logout-form">
                            @csrf
                            <button type="button" onclick="confirmLogout()" class="bg-white hover:bg-gray-100 text-black px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 shadow-lg hover:shadow-xl">
                                Logout
                            </button>
                        </form>
                    </div>
        ...
        <!-- Footer -->
        <footer class="bg-black/50 backdrop-blur-md border-t border-gray-800 py-4">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
